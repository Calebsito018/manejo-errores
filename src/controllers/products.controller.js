...
const getById = async (req, res) => {
    ...
    try {
        ...
        if (!product) {
            ...
            console.log(CustomError.createError(ErrorCause.PRODUCT_DATA_NOT_FOUND_IN_DATABASE))
...
const create = async (req, res) => {
    ...
    if (!title || !description || !price || !code || !stock || !category) {
        ...
        console.log(CustomError.createError(ErrorCause.PRODUCT_DATA_INCOMPLETE));
    ...
    if (codeExists) {
        ...
        console.log(CustomError.createError(ErrorCause.PRODUCT_DATA_CODE_ALREADY_EXISTS_IN_DATABASE));
        return res.status(400).send({ message: error });
    }
    try {
